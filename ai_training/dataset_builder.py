...
def process_record(record_name):
    ...
    try:
        ...
        for i in range(0, len(signal) - window_samples, step_samples):
            ...
            ann_indices = [idx for idx, samp in enumerate(annotation.sample) 
            ...
            mapped_labels = [LABEL_MAP.get(l) for l in labels_in_window if l in LABEL_MAP]
...
def build_dataset():
    ...
    for cls in required_classes:
        ...
        if count < min_samples:
            ...
            print(f"Class '{cls}' has only {count} samples. Augmenting with {needed} synthetic samples.")
    ...
    if needed_augmentation:
        ...
        for cls, count in synthetic_samples_needed.items():
            ...
            for _ in range(count):
                aug_data.append({
                    'heart_rate_bpm': np.max([0, np.random.normal(hr_mean, hr_std)]),
                    'hrv_sdnn_ms': np.max([0, np.random.normal(hrv_mean, hrv_std)]),
                    'pulse_amplitude': np.max([0, np.random.normal(amp_mean, amp_std)]),
                    'label': cls
                })
