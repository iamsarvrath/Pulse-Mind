...
class RhythmClassifier:
    ...
    def __init__(self):
        ...
        self.model_path = os.path.join(os.path.dirname(__file__), 'models', 'pulsemind_rf_model.pkl')
    ...
    def create_default_model(self):
        ...
        X_train = np.array([
            ...
            [70, 50, 25], [75, 55, 28], [65, 48, 22], [80, 52, 26],
            # Tachycardia (HR >100, low HRV, variable pulse)
            [110, 20, 18], [120, 15, 20], [105, 22, 19], [115, 18, 21],
            # Bradycardia (HR <60, variable HRV, weak pulse)
            [45, 35, 15], [50, 40, 12], [48, 38, 14], [52, 42, 16],
            # Irregular (variable HR, very low HRV, variable pulse)
            [85, 12, 20], [90, 10, 22], [88, 11, 19], [92, 13, 21],
            # Artifact (extreme values, very low HRV, very low pulse)
            [150, 5, 8], [30, 8, 6], [140, 6, 7], [35, 7, 9]
        ])
        
        y_train = np.array([
            0, 0, 0, 0,  # normal_sinus
            1, 1, 1, 1,  # tachycardia
            2, 2, 2, 2,  # bradycardia
            3, 3, 3, 3,  # irregular
            4, 4, 4, 4   # artifact
...
def classify_rhythm(
    ...
    """
    logger.info(f"Classifying rhythm: HR={heart_rate_bpm}, HRV={hrv_sdnn_ms}, Pulse={pulse_amplitude}")
    ...
    logger.info(f"Classification: {rhythm_class} (confidence: {confidence:.2f}, {inference_time_ms:.2f}ms)")
