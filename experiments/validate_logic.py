...
def validate_pipeline():
    ...
    with open(PIPELINE_OUT, "w") as f:
        ...
        resp = requests.post(
            f"{SERVICES['signal']}/process",
            json={"signal": sig_data, "sampling_rate": 100},
            timeout=5,
        )
        data = resp.json()
        ...
        # Assuming HSI service is mostly stateless transformations,
        # skipping profound test here
        ...
        resp_ai = requests.post(
            f"{SERVICES['ai']}/predict", json={"features": valid_feats}, timeout=5
        )
        ...
        if ai_data.get("success"):
            ...
            log_result(
                f, "AI: Label Exists", "rhythm_class" in pred, pred.get("rhythm_class")
            )
        else:
            log_result(f, "AI: Valid Input", False, resp_ai.text)

        # Noisy input
        resp_ai_bad = requests.post(
            f"{SERVICES['ai']}/predict",
            json={"features": {"heart_rate_bpm": -1}},
            timeout=5,
        )  # Invalid
        log_result(
            f,
            "AI: Invalid Input Handling",
            resp_ai_bad.status_code == 400,
            "Expect 400",
        )
    ...
    with open(CONTROL_OUT, "w") as f:
        ...
        for sc in scenarios:
            resp = requests.post(
                ...
                json={"rhythm_data": sc["rhythm"], "hsi_data": sc["hsi"]},
                timeout=5,
            )
            res = resp.json()
            ...
            f.write(
                f"| {sc['name']} | {status_icon} | "
                f"{pacing.get('pacing_mode')} | "
                f"State={pacing.get('safety_state')} |\n"
            )
