...
class TestPulseMindIntegration(unittest.TestCase):
    ...
    def test_04_database_persistence(self):
        ...
        # Since we run tests on localhost, the DB is in services/control-engine/pacing_decisions.db
        import sqlite3
        import os
        
        db_path = os.path.join("services", "control-engine", "pacing_decisions.db")
        ...
        try:
            ...
            logger.info(f"   -> Found {count} decisions in database")
